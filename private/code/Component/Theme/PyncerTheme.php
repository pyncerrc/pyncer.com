...
<?php
    ...
    private function makeHeader(array $page): string
    {
        ...
        if (pyncer_http_url_equals(
            ...
            ob_start();
?>
    <svg class="w-5 h-5 lg:w-6 lg:h-6 mr-1" viewBox="0 0 50 50" fill="currentColor" aria-hidden="true">
...
<?php
        ...
        ob_start();
?>
    <header class="flex sticky top-0 z-30 items-center bg-stone-50 dark:bg-stone-900 border-b border-stone-300 dark:border-stone-700 px-2 py-1 min-h-9">
        <h1 class="grow text-xl lg:text-2xl text-red-600 flex items-center">
            <?= $icon ?>

            <?= pyncer_he($page['header']['title']) ?>

            <?php if ($version !== null && $version !== '') { ?>
            <div class="text-xs text-red-600 ml-2 border border-red-600 radius rounded p-1">
                <?= pyncer_he($version) ?>
            </div>
            <?php } ?>
        </h1>
        ...
        <div class="flex pr-7 lg:pr-0">
            ...
            <div class="toggle relative" data-controller="locale">
                <button class="text-red-600 hover:text-red-400 transition-all duration-500 block w-5 h-5 lg:w-6 lg:h-6"
                    type="button"
                    aria-haspopup="true"
                    aria-expanded="false"
                ...
                >
                    <svg class="w-5 h-5 lg:w-6 lg:h-6" viewBox="0 0 50 50" fill="currentColor">
                ...
                <ul class="
                        absolute hidden top-[2.35rem] -right-14 min-w-[6rem] bg-stone-50 dark:bg-stone-900 border border-stone-300 dark:border-stone-700 py-1 rounded
                        xs:-right-[4.75rem]
                        lg:top-[2.6rem] lg:-right-16
                    "
                    id="theme_list"
            ...
            <div class="toggle relative" data-controller="theme"
                data-theme-selected-class="bg-stone-200 dark:bg-stone-800"
            >
                <button class="text-red-600 hover:text-red-400 transition-all duration-500 block w-5 h-5 lg:w-6 lg:h-6 ml-2"
                    type="button"
                    aria-haspopup="true"
                    aria-expanded="false"
            ...
            <a class="hidden xs:block text-red-600 hover:text-red-400 transition-all duration-500 w-5 h-5 lg:w-6 lg:h-6 ml-2" href="<?= pyncer_he(DOCS_GITHUB_URL) ?>" target="_blank">
                <span class="sr-only">
                    <?= pyncer_he($i18n->get('github.label')) ?>
                </span>
                <svg class="w-5 h-5 lg:w-6 lg:h-6 fill-current" viewBox="0 0 16 16" aria-hidden="true">
...
<?php
    ...
    private function makeNav(array $page): string
    {
        ...
        ob_start();
?>
    <nav class="toggle" role="navigation" data-controller="nav">
        <input
            class="hidden"
            type="checkbox"
            id="menu_toggle"
        ...
        <ul class="
                flex flex-col bg-stone-50 dark:bg-stone-900 border-b border-stone-300 dark:border-stone-700 absolute w-full p-2
                lg:fixed lg:left-0 lg:top-10 lg:bottom-0 lg:overflow-y-auto lg:w-44 lg:border-t lg:border-b-0
            "
            id="nav"
        >
<?php
...
?>
        <nav class="mb-2" aria-label="<?= pyncer_he($ariaLabel) ?>">
            <ol class="text-xs flex flex-wrap pb-2">
<?php
        foreach ($page['breadcrumb'] as $item) {
            $url = pyncer_http_relative_url($item['url']);
?>
                <li class="after:content-['/'] after:px-1 flex">
                    <a class="text-red-600 text-xs hover:underline underline-offset-8" href="<?= pyncer_he($url) ?>">
                        <?= pyncer_he($item['title']) ?>
                    </a>
                </li>
<?php
...
<?php
    ...
    private function makeMain(array $page): string
    {
        ...
        $main = str_replace(
            ...
            [
                '<h2 class="text-2xl mb-4 mt-6 first:mt-0 font-semibold">',
                '<p class="mb-4 max-w-xl">',
                '<a class="text-red-600 underline underline-offset-8 hover:no-underline" ',
        ...
        ob_start();
?>
    <main class="grow pl-8 pr-2 py-2 lg:ml-44 lg:border-stone-300 dark:lg:border-stone-700" id="main">
        <?= $this->makeI18nNotice() ?>
...
<?php
    ...
    private function makeI18nNotice(): string
    {
        ...
        ob_start();
?>
        <div class="border rounded border-red-600 flex items-center mb-2 max-w-xl">
